...
import { startAnalysisChat, continueAnalysisChat } from '../services/geminiService';
...
const parseMarkdown = (text: string): string => {
    ...
    // If the text looks like a JSON object containing "analysisReport", try to extract just that value.
    if (processingText.includes('"analysisReport":')) {
        // Try regex extraction first to grab the content inside the value quotes
        const match = processingText.match(/"analysisReport"\s*:\s*"([\s\S]*?)(?=",\s*"suggestedQuestions"|"\s*\}|$)/);
        if (match && match[1]) {
            processingText = match[1];
        } else {
            // Fallback cleanups for partials
            processingText = processingText
                .replace(/^[\s\S]*"analysisReport":\s*"/, '') // Remove start key
                .replace(/",\s*"suggestedQuestions"[\s\S]*$/, '') // Remove following keys
    ...
    let html = processingText
        ...
        .replace(/^[\s\n]*[{[,]\s*/, '') // Remove stray starting JSON brackets/braces
        .replace(/[\s\n]*[}\],]\s*$/, '') // Remove stray ending JSON brackets/braces
        .trim();
    ...
    // Tables with glass-like styling
    html = html.replace(/^\|(.+)\|\n\|( *[-:]+ *\|)+([\s\S]*?)(?=\n\n|\n*$)/gm, (match) => {
    ...
    // Headings with clear hierarchy and spacing
    ...
    // Highlights / Bold - The "Highlighter" Effect
    ...
    // Paragraphs with optimal reading measure
    const blocks = html.split(/\n\s*\n/);
    html = blocks.map(block => {
        if (block.match(/^<(div|ul|ol|table|h[1-6])/)) return block;
        // Use replace to handle internal line breaks within a paragraph properly
...
const AiChatWindow = ({ context, onClose, messages, setMessages, isMinimized, onToggleMinimize, isMaximized, onToggleMaximize }: AiChatWindowProps) => {
    ...
    // Styles
    const minimizedClass = "fixed bottom-6 right-6 w-80 h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 dark:border-slate-700 z-[100] flex items-center px-4 cursor-pointer transition-all duration-300 hover:scale-105 group";
    const maximizedClass = "fixed inset-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/20 dark:border-slate-700 flex flex-col z-[100] transition-all duration-500";
    const defaultClass = "fixed bottom-6 right-6 w-[500px] h-[750px] max-h-[85vh] bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl border border-white/20 dark:border-slate-700 flex flex-col z-[100] transition-all duration-500";

    if (isMinimized) {
        return (
            <div className={minimizedClass} onClick={onToggleMinimize}>
    ...
    return (
        <div className={isMaximized ? maximizedClass : defaultClass}>
            {/* Elegant Header */}
            <header className="flex items-center justify-between px-8 py-5 border-b border-slate-100/50 dark:border-slate-800/50 flex-shrink-0 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm rounded-t-[2.5rem]">
                ...
                <div className="flex items-center gap-1 bg-slate-100/50 dark:bg-slate-800/50 p-1.5 rounded-xl backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50">
                    <button onClick={onToggleMaximize} className="p-2 rounded-lg text-slate-500 hover:text-indigo-600 hover:bg-white dark:hover:bg-slate-700 transition-all" title={isMaximized ? "Restore" : "Maximize"}>
            ...
            {/* Chat Body */}
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-10 custom-scrollbar scroll-smooth bg-slate-50/30 dark:bg-slate-900/20">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex gap-5 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} animate-fade-in-up group`}>
                        
                        {/* Avatar */}
                        <div className={`flex-shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center shadow-sm mt-1 transition-transform group-hover:scale-105 ${
                            msg.role === 'user' 
                        ...
                        {/* Message Bubble */}
                        <div className={`max-w-[90%] shadow-sm transition-all duration-300 ${
                            msg.role === 'user' 
                            ? 'bg-slate-900 dark:bg-indigo-600 text-white rounded-2xl rounded-tr-sm p-4 md:p-5 shadow-lg shadow-slate-500/10' 
                            : 'bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-white/60 dark:border-slate-700/60 rounded-2xl rounded-tl-sm p-6 md:p-8 text-slate-700 dark:text-slate-200 shadow-xl shadow-indigo-500/5'
                        }`}>
                            {msg.role === 'user' 
                                ? <p className="text-sm md:text-base leading-relaxed font-medium">{msg.content}</p> 
                                : <div className="prose prose-slate dark:prose-invert max-w-none font-sans" dangerouslySetInnerHTML={{ __html: parseMarkdown(msg.content) }} />
                            }
                        </div>
                    </div>
                ))}
            ...
            <div className="p-6 absolute bottom-0 left-0 right-0 pointer-events-none bg-gradient-to-t from-white via-white/80 to-transparent dark:from-slate-900 dark:via-slate-900/80 pt-20 rounded-b-[2.5rem] z-10">
                <div className="pointer-events-auto space-y-4 max-w-3xl mx-auto">
                    {/* Suggestion Chips */}
                    {suggestedQuestions.length > 0 && !isLoading && (
                        <div className="flex gap-2 overflow-x-auto pb-2 px-1 custom-scrollbar no-scrollbar snap-x">
                            {suggestedQuestions.map((q, i) => (
                                <button
                                    key={i}
                                    onClick={() => setUserInput(q)}
                                    className="snap-start shrink-0 px-4 py-2.5 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl border border-indigo-100 dark:border-slate-700 text-indigo-600 dark:text-indigo-300 text-xs font-bold rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-0.5 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all duration-300 whitespace-nowrap flex items-center gap-2"
                                >
