...
import { StudentSF1 } from '../types';
import { saveStudents_SF1, loadStudents_SF1, logActivity, sendNotification } from '../services/databaseService';
...
const StudentRegistration = ({ user }: { user: firebase.User }) => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (editingStudentId) {
      setStudents(prev => prev.map(s => s.id === editingStudentId ? { ...s, ...currentStudent, age: calculateAge(currentStudent.birthDate) } : s));
      // Trigger local notification
      sendNotification(user.uid, {
          title: 'Student Updated',
          message: `Changes to ${studentName} have been staged. Remember to click "Save All Changes".`,
          type: 'info'
      });
    } else {
      ...
      sendNotification(user.uid, {
          title: 'Student Added',
          message: `${studentName} has been added to the list.`,
          type: 'success'
      });
    }
  ...
  const executeDelete = () => {
      ...
      if (studentId) {
        setStudents(prev => prev.map(s => s.id === studentId ? { ...s, deletedAt: Date.now() } : s));
        sendNotification(user.uid, {
            title: 'Student Deleted',
            message: `${studentName} moved to trash. Will be permanently removed in 7 days.`,
            type: 'warning'
        });
      }
  ...
  const renderForm = () => (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4 transition-all">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-700 animate-fade-in-up">
            ...
            <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto px-8 py-6 space-y-8 custom-scrollbar bg-slate-50/50 dark:bg-slate-900/50">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        ...
                        <div className="md:col-span-2">
                            ...
                            <input type="text" name="motherTongue" value={currentStudent.motherTongue} onChange={handleInputChange} className="w-full input-field" />
                        </div>
                        ...
                        <div>
                            <label htmlFor="birthDate" className="block text-xs font-semibold text-slate-500 uppercase mb-1.5">Birth Date</label>
                ...
                <div>
                    ...
                    <input type="text" name="remarks" value={currentStudent.remarks} onChange={handleInputChange} className="w-full input-field" placeholder="Transferred In/Out, CCT, etc." />
                </div>
            ...
            <footer className="flex justify-end px-8 py-5 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 gap-3">
                <button onClick={handleCloseForm} type="button" className="px-6 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-colors">
                 ...
                 <button onClick={handleFormSubmit} type="submit" className="flex items-center px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-indigo-500/20 transition-all hover:-translate-y-0.5">
                    {editingStudentId ? <EditIcon className="w-4 h-4 mr-2"/> : <PlusIcon className="w-4 h-4 mr-2"/>}
